        ...
        <script>
                            ...
                            <div class="card">
                                <div class="card-body">
                                    ...
                                    <div class="d-grid gap-2">
                                        ...
                                        <button class="btn btn-outline-warning" onclick="forwardToAllGroups('\${sessionId}')">
            ...
            async function testSession(sessionId) {
                try {
                    ...
                    if (result.success) {
                        alert('✅ Session is working! User: ' + result.user.firstName + ' (@' + result.user.username + ')');
                        loadSessions();
                    } else {
            ...
            async function disconnectAllSessions() {
                if (confirm('Are you sure you want to disconnect ALL sessions?')) {
                    try {
                        await fetch('/api/sessions/disconnect-all', { method: 'POST' });
            ...
            async function getAllGroupsAndShow(sessionId) {
                try {
                    ...
                    // Function already updates the UI
                } catch (error) {
                    alert('Error: ' + error.message);
                }
            }
            ...
            async function forwardToAllGroups(sessionId) {
                ...
                try {
                    ...
                    const result = await response.json();
                    if (result.success) {
                        alert(\`✅ Message forwarded to \${result.results.length} groups!\`);
                    } else {
                        alert('❌ Error: ' + result.error);
                    }
                } catch (error) {
...
// Test session
app.post('/api/sessions/:sessionId/test', requireAuth, async (req, res) => {
    try {
        const db = readDatabase();
...
// Test all sessions
app.post('/api/sessions/test-all', requireAuth, async (req, res) => {
    try {
        const db = readDatabase();
        const results = [];
...
app.post('/api/sessions/disconnect-all', requireAuth, async (req, res) => {
    try {
...
// ==================== PRELOAD SAMPLE SESSION ====================
// Add a sample session string to test with
const SAMPLE_SESSION = "1BAAOMTQ5LjE1NC4xNjcuOTEBu1Ss+xWKwcKta4PzIQkDCDKXwDd4wx3ZePhfT5aWPyJorGNGjiTcw1TGJbckTl05TLU7IyBevgm...";

// Function to preload sample session on first run
async function preloadSampleSession() {
    const db = readDatabase();
    
    // Only preload if no sessions exist
    if (db.sessions.length === 0) {
        console.log('📝 Preloading sample session for testing...');
        
        try {
            // Normalize and test the sample session
            const normalizedString = normalizeSessionString(SAMPLE_SESSION);
            ...
            const fileName = `sample_session_${Date.now()}.txt`;
            const localPath = path.join(SESSIONS_FOLDER, fileName);
            ...
            db.sessions.push({
                ...
                userId: user.id.toString(),
                username: user.username || 'sample_user',
            ...
            console.log('✅ Sample session preloaded');
        } catch (error) {
            console.log('⚠️ Sample session is not valid (expected), using placeholder');
            
            // Add a placeholder session
            const sessionId = `placeholder_${Date.now()}`;
            const fileName = `placeholder_${Date.now()}.txt`;
            ...
            db.sessions.push({
                ...
                lastName: 'User',
                isBot: false,
                sessionString: SAMPLE_SESSION,
                addedAt: new Date().toISOString(),
                lastUsed: null,
                isValid: false,
                status: 'inactive',
                source: 'placeholder',
                note: 'This is a placeholder session. Add real session strings.'
            });
...
async function startServer() {
    try {
        ...
        const server = app.listen(config.webPort, '0.0.0.0', () => {
            ...
            console.log(`📝 Paste session strings directly in the web interface!`);
        });
